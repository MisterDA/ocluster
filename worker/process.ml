...
let exec ~label ~log ~switch ?env ?(stdin="") ?(stderr=`FD_copy Unix.stdout) ?(status=(function n -> n = 0)) cmd =
...
let check_call ~label ~log ~switch ?env ?stdin ?stderr ?status cmd =
  exec ~label ~log ~switch ?env ?stdin ?stderr ?status cmd >|= function
  | Ok () -> Ok ()
